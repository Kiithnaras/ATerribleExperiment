...
function getCompletion(s)
	if senses_only then
		if OptionsManager.isOption("SYSTEM", "pf") then
			if string.lower(s) == string.lower(string.sub("Notice", 1, #s)) then
				return string.sub("Notice", #s + 1);
			end
		else
			if string.lower(s) == string.lower(string.sub("Listen", 1, #s)) then
				return string.sub("Listen", #s + 1);
			end
			if string.lower(s) == string.lower(string.sub("Spot", 1, #s)) then
				return string.sub("Spot", #s + 1);
			end
		end
	else
		-- Find a matching completion for the given string
		for k,_ in pairs(GameSystemManager.getSkillList()) do
...
function action(draginfo)
	if sDragLabel then
		...
		ActionSkill.performRoll(draginfo, rActor, sDragLabel, nDragMod);
	end
end
