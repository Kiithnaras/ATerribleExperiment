...
function registerChangeCallback(fCallback)
	if not nodeCalendar then
		nodeCalendar = DB.createNode("calendar.data");
		nodeCalendar.onChildAdded = onCalendarChanged;
	end
...
function outputDate()
	local msg = {sender = "GM", font = "chatfont", icon = "portrait_gm_token", mode = "chat"};
	msg.text = "The date is the " .. getCurrentDateString();
	Comm.deliverChatMessage(msg);
end

function outputTime()
	local msg = {sender = "GM", font = "chatfont", icon = "portrait_gm_token", mode = "chat"};
	msg.text = "The time is " .. getCurrentTimeString();
	Comm.deliverChatMessage(msg);
end
