...
function onInit()
	...
	DB.addHandler("combattracker.list.*.nonlethal", "onUpdate", updateHealth);
...
function updateTooltip(tokenCT, nodeCT)
	...
	if sOptTH == "tooltip" then
		local sStatus;
		_, _, sStatus = ActorManager2.getPercentWounded("ct", nodeCT);
...
function updateHealthBarScale(tokenCT, nodeCT)
	...
	if widgetHealthBar then
		local nPercentWounded, nPercentNonlethal = ActorManager2.getPercentWounded("ct", nodeCT);
