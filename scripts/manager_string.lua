...
function convertStringToDice(s)
	...
	if s then
		for sSign, v in s:gmatch("([+-]?)([%dd]+)") do
			...
			-- DIE REFERENCE
			local sDieCount, sDieType = v:match("^(%d*)(d%d+)");
			if sDieType then
...
function convertAbilityWordArrayToEffectString(aAbilities, bPenalty)
	...
	for keyAbility, sAbility in pairs(aAbilities) do
		...
		if sAbility:sub(1, 4) == "half" then
			sEffectAbility = sEffectAbility .. "H";
			sAbility = sAbility:sub(5);
		elseif sAbility:sub(1, 6) == "double" then
			sEffectAbility = sEffectAbility .. "2";
			sAbility = sAbility:sub(7);
		end
		
		if sAbility == "level" then
			sEffectAbility = sEffectAbility .. "LVL";
		else
			sEffectAbility = sEffectAbility .. string.upper(sAbility:sub(1, 3));
		end
...
function evalDiceString(sDice, bAllowDice, bMaxDice)
	...
	for sSign, sVal, sDieType in sDice:gmatch("([%-%+]?)(%d+)d?(%d*)") do
		local nVal = tonumber(sVal) or 0;
		local nSubtotal = 0;

		if sDieType ~= "" then
			if bAllowDice then
				local nDieSides = tonumber(sDieType) or 0;
				if nDieSides > 0 then
					if bMaxDice then
...
function evalDice(aDice, nMod, bCritical)
	...
	for _,sDie in pairs(aDice) do
		local nDieSides = tonumber(sDie:match("d(%d+)")) or 0;
		if nDieSides > 0 then
			if bCritical then
