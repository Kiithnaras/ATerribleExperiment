...
function toggle(sClassWin, nodeWin)
	local wnd = Interface.findWindow(sClassWin, nodeWin);
	if wnd then
		local bMinimized = wnd.isMinimized();
		if bMinimized then
			wnd.bringToFront();
		else
			wnd.close();
		...
		wnd = Interface.openWindow(sClassWin, nodeWin);
	end
...
function onWindowOpened(window)
	...
	if CampaignRegistry then
		...
		if nodeWin then
			sourcename = nodeWin.getNodeName();
		end
