...
function onInit()
	ActionsManager.registerModHandler("table", modTable);
	ActionsManager.registerResultHandler("table", onTableRoll);
	
	Comm.registerSlashHandler("rollon", processTableRoll);
end

function performRoll(draginfo, rActor, nodeTable, nColumn, bUseModStack, aDice, nMod, bHide)
	...
	if bHide then
		rRoll.sDesc = rRoll.sDesc .. " [HIDDEN]";
	end
	rRoll.aDice = aDice;
	rRoll.nMod = nMod;
	...
	-- Perform the roll
	ActionsManager.performSingleRollAction(draginfo, rActor, "table", rRoll);
end
...
function modTable(rSource, rTarget, rRoll)
	return true;
end
...
function onTableRoll(rSource, rTarget, rRoll, rCustom)
	...
	if sColumn then
		...
		-- nColumn = findColumn(nodeTable, sColumn);
	end
	...
	local bHideRoll = (DB.getValue(nodeTable, "hiderollresults", 0) ~= 0);
	if string.match(rMessage.text, "%[HIDDEN%]") then
		bHideRoll = true;
		rMessage.text = string.gsub(rMessage.text, " %[HIDDEN%]", "");
	end
	if bHideRoll then
		rMessage.text = rMessage.text .. " [HIDDEN]";
	end
...
function processTableRoll(sCommand, sParams)
	...
	TableManager.performRoll(nil, nil, nodeTable, nColumn, false, aDice, nMod, bHide);
end
